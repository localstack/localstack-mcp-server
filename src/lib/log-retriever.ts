...
export class LocalStackLogRetriever {
  ...
  async retrieveLogs(lines: number = 10000, filter?: string): Promise<LogRetrievalResult> {
      ...
      if (errorMessage.includes("timeout")) {
        return {
          ...
          logs: [],
          totalLines: 0,
          errorMessage: "Log retrieval timed out. LocalStack may be generating large amounts of logs. Try reducing the number of lines or check if LocalStack is experiencing issues.",
        };
      }
      ...
      if (errorMessage.includes("Command failed") || errorMessage.includes("not found")) {
        return {
          ...
          logs: [],
          totalLines: 0,
          errorMessage: "Unable to execute 'localstack logs' command. Please ensure LocalStack CLI is installed and LocalStack is running.",
        };
      }
  ...
  private parseLogLine(line: string): LogEntry {
    ...
    const localstackApiMatch = line.match(/AWS\s+([a-z0-9_-]+)\.([A-Za-z]+)\s*=>\s*(\d{3})\s*(?:\(([^)]+)\))?/);
    ...
    if (!entry.operation) {
      ...
      for (const pattern of operationPatterns) {
        const match = line.match(pattern);
        if (match && match[1].length > 2 && !['INFO', 'WARN', 'ERROR', 'DEBUG', 'TRACE'].includes(match[1])) {
