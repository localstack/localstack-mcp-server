...
export const schema = {
  action: z
    ...
    .describe("The App Inspector action to perform"),
  status: z
    .enum(["enabled", "disabled"])
    .optional()
    .describe("Status to set (required for set-status)"),
  trace_id: z
    .string()
    .optional()
    .describe("Trace ID (required for get-trace, list-spans, get-span, delete-spans, list-events, list-iam-events)"),
  span_id: z
    .string()
    .optional()
    .describe("Span ID (required for get-span, list-events, list-iam-events)"),
  trace_ids: z
    .array(z.string())
    .optional()
  ...
  span_ids: z
    .array(z.string())
    .optional()
    .describe("Span IDs to delete (for delete-spans; omit to delete all spans in the trace)"),
  limit: z
    .number()
    .int()
    .min(1)
  ...
  service_name: z
    .string()
    .optional()
    .describe("Filter by service name (e.g., 'lambda', 's3')"),
  region: z
    .string()
    .optional()
    .describe("Filter by AWS region (e.g., 'us-east-1')"),
  account_id: z
    .string()
    .optional()
    .describe("Filter by AWS account ID"),
  operation_name: z
    .string()
    .optional()
    .describe("Filter by operation name (e.g., 'CreateBucket')"),
  resource_name: z
    .string()
    .optional()
    .describe("Filter by resource name"),
  arn: z
    .string()
    .optional()
    .describe("Filter by resource ARN"),
};
...
export default async function localstackAppInspector(
  params: InferSchema<typeof schema>
) {
  const { action } = params;
  return withToolAnalytics("localstack-app-inspector", { action }, async () => {
...
async function handleGetStatus() {
  const data = await httpClient.request<{ status: string; note?: string }>(
    `${API_PREFIX}/status`
  );
...
async function handleListTraces(params: InferSchema<typeof schema>) {
  const qs = buildQueryString(buildFilters(params));
...
async function handleListSpans(params: InferSchema<typeof schema>) {
  ...
  const data = await httpClient.request<any>(
    `${API_V1}/traces/${params.trace_id}/spans${qs}`
  );
...
function buildFilters(params: InferSchema<typeof schema>): Record<string, string | number | undefined> {
  return {
    limit: params.limit,
...
function formatTraceList(data: any): string {
  ...
  const rows = traces
    .map((t: any) => {
      ...
      const operation = t.operation_name ?? "-";
...
function formatSpanList(data: any, traceId: string): string {
...
function formatEventList(data: any, spanId: string, prefix = ""): string {
