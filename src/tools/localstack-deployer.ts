...
export const schema = {
  action: z.enum(['deploy', 'destroy']).describe("The deployment action to perform: 'deploy' to create/update resources, or 'destroy' to remove them."),
  projectType: z.enum(['cdk', 'terraform', 'auto']).default('auto').describe("The type of project. 'auto' (default) infers from files. Specify 'cdk' or 'terraform' to override."),
  directory: z.string().describe("The required path to the project directory containing your infrastructure-as-code files."),
  variables: z.record(z.string()).optional().describe("Key-value pairs for parameterization. Used for Terraform variables (-var) or CDK context (-c)."),
};
...
// Tool implementation
export default async function localstackDeployer({ action, projectType, directory, variables }: InferSchema<typeof schema>) {
...
async function executeTerraformCommands(
  ...
  const varArgs = variables ? Object.entries(variables).map(([key, value]) => `-var="${key}=${value}"`).join(' ') : '';
  ...
  if (action === 'deploy') {
    ...
    try {
      ...
      const errorOutput = error instanceof Error && 'stderr' in error ? stripAnsiCodes(String((error as any).stderr)) : String(error);
      result += `\`\`\`\n${errorOutput}\n\`\`\`\n`;
...
async function executeCdkCommands(
  ...
  const contextArgs = variables ? Object.entries(variables).map(([key, value]) => `--context ${key}=${value}`).join(' ') : '';

  if (action === 'deploy') {
    result += `## 🥾 Bootstrapping CDK for LocalStack\n\n`;
    try {
      const { stdout: bootstrapStdout, stderr: bootstrapStderr } = await execAsync(`${baseCommand} bootstrap`, { 
        cwd: directory,
        timeout: 120000,
        maxBuffer: 1024 * 1024 * 10,
